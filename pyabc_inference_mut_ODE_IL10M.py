...
import numpy as np
from src.models.IL10M_RAp_ODE import model_function as model_function_IL10_RAp
import pandas as pd
...
df_bind = pd.read_csv(path_data+'/IL10/data/binding/IL10_data_param_ABC_SMC_IL10_RAp.csv')
...
df_sim_data = pd.read_csv(path_data+"/IL10/data/signaling/IL10M_STAT_data_Gorby.tsv.gz", sep='\t', compression='gzip')
df_EC50_AMP = pd.read_csv(path_data+'/IL10/data/signaling/IL10M_EC50_AMP_Gorby.tsv.gz', sep="\t", compression="gzip")
...
ABC_SMC_path = "pyABC_inference_ODE_mut_IL10M.db"
conf95_interval = 1.25 #1.5 is to much since a lot of the population is out of bounds -> algorithm can't decrease the epsilon
pop_size_init = 100 
pop_size_max = 250
E_cv = 0.275 # Use 0.25 as maximum if the simulations taexit
ke too long. 0.15 -> 300 indiv, 0.2 -> 100 indiv
initial_epsilon = 0.75
min_epsilon = 5e-5
max_num_iter = 5 # Use 10 for variants where only one parameter needs to be infered
model_function_dict = {
