...
Rule('IL_RA1p_RA2_noRB1_binding_TB2', IL_RA1p_RA2 + RB(bB=None) | IL_RA1p_RA2_RB1, *[K_IL_RA_RB_f, K_IL_RA_RB_3_b])
...
# Inputs
graph_gen_species = model.species[:3]+[model.species[10]] # model.species[1:4] -> for the model of dimeric IL10, model.species[:3] -> for model monomeric IL10
...
# Combine edges with the same directions
i = 0
edge_direc = []
edge_direc_index = []
repeated_edges = []
for edge in graph_final:
    if [edge[0],edge[2]] not in edge_direc:
        edge_direc.append([edge[0],edge[2]])
        edge_direc_index.append(i)
    else:
        j = 0
        for edge_direc_repeat in edge_direc:
            if edge_direc_repeat == [edge[0],edge[2]]:
                repeated_edges.append([j,i])
            j += 1
    i += 1
graph_reduced = []
i = 0
for edge in graph_final:
    if i in [elem[0] for elem in repeated_edges]:
        graph_reduced.append((edge[0],edge[1].replace("-RA","-RA__RAp"),edge[2]))
    elif i not in [elem[1] for elem in repeated_edges] + [elem[0] for elem in repeated_edges]:
        graph_reduced.append(edge)
    i += 1
dict_repeated_weights_names['RA__RAp'] = '(RA+RAp)'
graph_final = graph_reduced
...
obj=writescripts.PrepareFilesNoneq(edgelist=graph_final,varGRF='IL10D',concvars=['RA','RB','RAp','RA__RAp'],parlist=parlist,MTTfolder=MTTfolder,graphbasename=basename)
...
with open("rhos_dimer_phos.pkl", "wb") as file:
