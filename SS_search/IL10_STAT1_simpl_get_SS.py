...
graph_final.append((1,'K_ILM_b',13))
graph_final.append((13,'K_ILM_f-IL10M',1))
new_index_species[12] = "ILM() ** EC"
...
with open("rhos_STAT1_simpl.pkl", "rb") as file:
...
rho_6 = sp.simplify(rho_list[5].split("=")[1]) # ILRA1RB1M1
rho_7 = sp.simplify(rho_list[6].split("=")[1]) # ILRA1RA2RB1M1
rho_8 = sp.simplify(rho_list[7].split("=")[1]) # ILRA1RA2RB1RB2
rho_9 = sp.simplify(rho_list[8].split("=")[1]) # ILRA1RB1M1S1
rho_10 = sp.simplify(rho_list[9].split("=")[1]) # ILRA1RA2RB1M1S1
rho_11 = sp.simplify(rho_list[10].split("=")[1]) # ILRA1RA2RB1RB2M1
rho_12 = sp.simplify(rho_list[11].split("=")[1]) # ILRA1RA2RB1RB2M1S1
rho_13 = sp.simplify(rho_list[12].split("=")[1]) # ILM_free

rho_t = rho_1+rho_2+rho_3+rho_4+rho_5+rho_6+rho_7+rho_8+rho_9+rho_10+rho_11+rho_12+rho_13
...
RA_expr = RA0-RA-(rho_2+2*rho_3+rho_4+2*rho_5+rho_6+2*rho_7+2*rho_8+rho_9+2*rho_10+2*rho_11+2*rho_12)/rho_t*IL10M0
RB_expr = RB0-RB-(rho_4+rho_5+rho_6+rho_7+2*rho_8+rho_9+rho_10+2*rho_11+2*rho_12)/rho_t*IL10M0
M_expr = M0-M-(rho_6+rho_7+rho_9+rho_10+rho_11+rho_12)/rho_t*IL10M0
S1_expr = S10-S1-(rho_9+rho_10+rho_12)/rho_t*IL10M0
IL_expr = IL10M0-IL10M-(rho_1+rho_2+rho_3+rho_4+rho_5+rho_6+rho_7+rho_8+rho_9+rho_10+rho_11+rho_12)/rho_t*IL10M0

ILRA1RB1M1S1 = S10-S1-(rho_10+rho_12)/rho_t*IL10M0
ILRA1RA2RB1M1S1 = S10-S1-(rho_9+rho_12)/rho_t*IL10M0
ILRA1RA2RB1RB2M1S1 = S10-S1-(rho_9+rho_10)/rho_t*IL10M0


with open("SS_expr_STAT1_simpl.pkl", "wb") as file:
    pickle.dump([str(RA_expr),str(RB_expr),str(M_expr),str(S1_expr),str(IL_expr),str(ILRA1RB1M1S1),str(ILRA1RA2RB1M1S1),str(ILRA1RA2RB1RB2M1S1)], file)
